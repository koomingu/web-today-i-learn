...
1. `attendance` 테이블은 중복된 데이터가 쌓이는 구조이다. 중복된 데이터는 어떤 컬럼인가?
    - 중복 컬럼: `crew_id`, `nickname`
2. `attendance` 테이블에서 중복을 제거하기 위해 `crew` 테이블을 만들려고 한다. 어떻게 구성해 볼 수 있을까?
    - 테이블명: `crew`
    - 컬럼 구성:
        - `crew_id`: 크루를 식별할 수 있는 고유 번호 (Primary Key)
        - `nickname`: 크루의 활동명 (Not Null)
...
1. crew 테이블을 만들고 중복을 제거했다. attendance에서 불필요해지는 컬럼은?
    - nickname
    - 이유
        - 중복 제거: 동일한 정보가 여러 곳에 저장되어 있으면 저장 공간이 낭비됩니다.
        - 데이터 무결성: 만약 크루가 닉네임을 변경했을 때, `crew` 테이블의 닉네임은 수정되었는데 `attendance` 테이블의 수많은 행 중 일부가 수정되지 않는다면 데이터 불일치 문제가 발생합니다.
        - 참조: `attendance` 테이블의 `crew_id`를 통해 `crew` 테이블과 조인하면 언제든지 닉네임을 가져올 수 있습니다.
...
```sql
-- 3월 4일 출석한 크루 중 '디'로 시작하는 크루 검색
SELECT c.crew_id, c.nickname
FROM attendance a
...
```sql
SELECT c.nickname, COUNT(a.start_time) AS actual_attendance_days
FROM attendance a
JOIN crew c ON a.crew_id = c.crew_id
GROUP BY c.nickname;
```
